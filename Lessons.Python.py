...
import pandas as pd
name = "AAAAAAAAAAAAAAAHHHHHHHHHHHHHHHHHHHH.txt"
variable = open(name)


f_name = open("AAAAAAAAAAAAAAAHHHHHHHHHHHHHHHHHHHH.txt")
for name in f_name:
    name = name.strip()
...
with open("AAAAAAAAAAAAAAAHHHHHHHHHHHHHHHHHHHH.txt","w") as f:   
    #f.write("Test")
    f.seek(1)
   # f.write("Test")

with open ("test_1.jpg","rb") as rf:  
...
df_2 = pd.read_csv("survey_results_public.csv")
df_2.loc[4:9, 'MainBranch':'ToolsTechHaveWorkedWith']
df_2.loc[4:9,'LessThanHighSchoolGraduate':'BachelorsDegreeorHigher']
def mapGeography(x):
    if x=="City":
        return 1
    else:
...
df3 = pd.read_csv('baby_names.csv')
df3.head(10)
df3 = df3.drop("Unnamed: 0",axis =1)
df3 = df3.drop("Id",axis =1)
df3["Gender"].value_counts()

print (len(df3["Gender"]=="F"))
print(df3)
names = df3.groupby("Name")
names = df3.drop('Year', axis = 1).groupby('Name').sum()
names.describe()
names

df3.groupby('Name').Count.sum().sort_values(ascending = False)

df4 = pd.read_csv('Euro.csv')

df4.sort_values("Yellow Cards", ascending = 0).head()
